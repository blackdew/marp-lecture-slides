...
### Agent가 모든 걸 해줘도, 선택은 사람의 몫

| Agent가 하는 것 | 사람이 하는 것 |
|---------------|--------------|
| 초안 3가지 버전 생성 | 어떤 버전을 쓸지 **선택** |
| 장단점 비교표 작성 | 최종 **결정** |
| 데이터 분석 결과 제시 | 결과를 **해석**하고 **판단** |

- Agent에게 "추천해줘"라고 하면 추천은 해줍니다
- 하지만 **왜 이걸 선택했는지 설명할 수 있어야** 합니다
- 과제든, 보고서든, 연구든 — **최종 책임은 나에게** 있습니다

<div class="highlight">

Agent는 선택지를 만들어주는 도구. <strong>고르고 책임지는 건 사람</strong>입니다.
...
| 단계 | 하는 일 | 예시 |
|------|---------|------|
| **1. 작업** | 매번 직접 시킨다 | "이번 주 회의록 정리해줘" |
| **2. 프로세스화** | 반복 패턴을 발견하고 기록한다 | "회의록은 항상 참석자, 안건, 결정사항, 액션아이템 순서로" |
| **3. 스킬화** | 프로세스를 한 마디로 실행한다 | "주간 회의록" → 자동으로 전체 과정 수행 |
...
| 키워드 | 핵심 메시지 |
|--------|-----------|
| **선택** | Agent가 만들어줘도, 고르고 책임지는 건 사람 |
| **플랜** | 컨텍스트를 채울수록 결과가 좋아진다 |
| **문서화** | 기록의 비용이 0이 된 시대, 먼저 기록하라 |
