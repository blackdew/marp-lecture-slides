...
| **선호 학습** | 사람 피드백으로 강화학습 (RLHF) |

**핵심:** 패턴 인식 + 사람의 선호를 반영한 응답 생성
...
**인간의 언어 발화:**
- 맥락에 따라 다음 단어 선택
- 모르면? → 그럴듯하게 채워넣기 (할루시네이션)
- 역할 바뀌면? → 말투와 사고 변화 (페르소나)
...
**컨텍스트 윈도우 관리:**
- 토큰 한계 초과 시 오래된 내용 제거
- 중요한 정보는 요약해서 보존

**실제 동작:**
- "아까 말한 그 함수"를 이해할 수 있는 이유
- 대화가 길어지면 초반 내용을 "잊어버림"

---

# Planning: 컨텍스트를 채우는 과정

**문제:** "보고서 만들어줘" → 무슨 보고서? 어떤 내용?

**Reasoning (추론):**
- 주어진 정보로 빠진 부분 추론
- "프로젝트 폴더가 있네 → 프로젝트 보고서구나"

**Planning (계획):**
- 목표 달성을 위한 단계 수립
- "1) 파일 탐색, 2) 내용 분석, 3) 보고서 작성"
- Chain-of-Thought로 단계별 실행

**핵심:** 부족한 컨텍스트를 스스로 채워서 완성
...
**GUI vs CLI:**
- GUI (Graphical): 버튼 클릭, 시각적 → 사람용
- CLI (Command Line): 텍스트 명령 → **LLM이 바로 사용!**

**CLI의 장점:**
- "git status" 같은 텍스트 명령
- LLM이 이미 잘 이해하는 형식
- **CLI 있으면 MCP 불필요**

**MCP (Model Context Protocol):**
- CLI 없는 서비스를 위한 표준 규약
- 예: 노션, 슬랙 등을 Agent가 사용 가능

**핵심:** LLM은 텍스트 기반 도구와 천생연분
...
- **LLM**: Sonnet 4.5 (최신 모델)
- **Memory**: 대화 전체 컨텍스트 유지
- **Planning**: **Plan 모드**로 사전 검토
- **Tools**: CLI, 파일, 검색 직접 실행

**Plan 모드의 힘:**
1. 컨텍스트 부족 → Reasoning으로 추론
2. 단계별 계획 수립 → 사용자에게 보여줌
3. 승인 후 실행 → **안전한 자동화**
...
1. LLM = 패턴 인식 + 사람 선호 학습 (RLHF)
2. 인간과 AI는 비슷하게 동작 → **컨텍스트가 핵심**
3. Agent = LLM + Memory + Planning + Tools
4. Memory: 맥락 저장 / Planning: 컨텍스트 채우기 / Tools: 실행
5. CLI는 LLM과 천생연분 (텍스트 기반)
6. 한계를 인정하고 **인간이 책임**

**다음 Part:**

계획: 명확한 컨텍스트 제공 기술 + 실습
...
| 모호한 지시 | 비어있는 것 | Agent의 추론 | 실제 의도 |
|------------|------------|-------------|-----------|
| "정리해줘" | 무엇을? 어떻게? | 안 쓰는 파일 삭제 | 폴더 구조 개선 |
| "최적화해줘" | 무엇을? 기준은? | 속도 최우선 | 메모리 사용량 |
| "업데이트해줘" | 뭘? 언제까지? | 최신 버전으로 | 안정 버전으로 |

**핵심:** 빈칸은 Agent가 **추측**으로 채움 → 위험
...
**1. 구체성 (Specificity)**
- 나쁜 예: "웹사이트 만들어줘"
- 좋은 예: "React로 할일 관리 웹앱"
- **차이**: 무엇을, 어떤 기술로 → 명확함

**2. 단계 (Steps)**
- 나쁜 예: "보고서 작성해줘"
- 좋은 예: "1) 완료 작업 2) 계획 3) 이슈"
- **차이**: 순서와 구조 → 따라갈 수 있음

**3. 검증 가능성 (Verifiable)**
- 나쁜 예: "코드 개선해줘"
- 좋은 예: "에러 핸들링 추가, async/await 적용"
- **차이**: 완료 여부 확인 가능 → 검증 가능
