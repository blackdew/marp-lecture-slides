...
이 저장소는 마크다운 기반 프레젠테이션을 사용하여 강의 자료를 생성하고 관리합니다. 모든 강의는 `lectures/` 디렉토리 아래에 위치하며 일관된 구조를 따릅니다.
...
- **Implementation Plan**: 모든 구현 계획(Implementation Plan)은 반드시 **한글**로 작성해야 합니다.
- **문서화**: 프로젝트 관련 모든 문서(커밋 메시지, PR 설명 등)는 한글 작성을 원칙으로 합니다.
...
```bash
./generate-presentation.sh lectures/<폴더명> <마크다운파일명>
```
...
marp lectures/<폴더>/<파일.md> --pdf       # Puppeteer 의존성 필요
marp lectures/<폴더>/<파일.md> --pptx      # 직접 PPTX 생성
```
...
1. `./generate-presentation.sh lectures/<폴더> <파일.md>`를 실행하여 HTML 재생성
...
git status
# HTML 출력물이 최신인지 확인
./generate-presentation.sh lectures/<폴더> <파일.md>
# 브라우저에서 변경사항 검토
```
